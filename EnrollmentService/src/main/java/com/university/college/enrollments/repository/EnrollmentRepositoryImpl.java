package com.university.college.enrollments.repository;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.jdbc.core.JdbcTemplate;
import org.springframework.stereotype.Repository;
import com.university.college.enrollments.irepository.IEnrollmentRepository;
...
public class EnrollmentRepositoryImpl implements IEnrollmentRepository {
  ...
  public List<Enrollment> GetEnrollmentByRollNumber(String rollNumber) {
    ...
    try {
      enrollments = _JdbcTemplate.query("select * from enrollment where s_id = ?", new EnrollmentRowMapper(),
          rollNumber);
    } catch (Exception e) {
  ...
  public List<Enrollment> GetEnrollmentByCourseId(String courseId) {
    List<Enrollment> enrollments = new ArrayList<>();
    try {
      enrollments = _JdbcTemplate.query("select * from enrollment where c_id = ?", new EnrollmentRowMapper(), courseId);
    } catch (Exception e) {
